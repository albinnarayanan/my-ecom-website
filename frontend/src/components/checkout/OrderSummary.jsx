                        ...
                        <div className="space-y-2">
                            {cart?.map((item)=>(
                                <div key={item.productId} className="flex items-center">
                                    <img src={`${import.meta.env.VITE_BACKEND_URL}/images/${
                                        item?.image
                                    }`} alt="Product" className="w-12 h-12 rounded" />
                                    <div className="text-gray-500 ">
                                        <p className="ml-2">{item?.productName}</p>
                                        <p className="ml-2">
                                            {item?.quantity} x ${item?.specialPrice} = ${formatPriceCalculation(item?.quantity,item?.specialPrice)}
                                        </p>
                                    </div>
                                </div>
                            ))}
